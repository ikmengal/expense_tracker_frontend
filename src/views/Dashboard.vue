        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 items-start">
          ...
          <div class="space-y-6">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-xs p-5 md:p-6 space-y-4">
              ...
              <form @submit.prevent="handleSubmitTransaction" class="space-y-4" novalidate>
                <div class="grid grid-cols-3 gap-2 p-1.5 bg-slate-100 rounded-2xl border border-slate-200/60">
                  <button 
                    @click="openLiveCamera" 
                    type="button" 
                    class="flex flex-col items-center justify-center py-2.5 bg-white rounded-xl shadow-xs border border-slate-200/40 hover:bg-indigo-50/50 group transition cursor-pointer"
                  >
                    <span class="text-lg group-hover:scale-110 transition duration-200">📷</span>
                    <span class="text-[10px] font-bold text-slate-600 mt-1 uppercase tracking-wider">Camera</span>
                  </button>

                  <label 
                    for="receipt-direct-upload" 
                    class="flex flex-col items-center justify-center py-2.5 bg-white rounded-xl shadow-xs border border-slate-200/40 hover:bg-indigo-50/50 group transition cursor-pointer text-center"
                  >
                    <span class="text-lg group-hover:scale-110 transition duration-200">📁</span>
                    <span class="text-[10px] font-bold text-slate-600 mt-1 uppercase tracking-wider">Receipt</span>
                    <input 
                      type="file" 
                      ref="receiptInput" 
                      @change="handleReceiptScan" 
                  ...
                  <button 
                    @click="toggleVoiceListening" 
                    type="button"
                    :class="isListening ? 'bg-rose-50 border-rose-200 ring-2 ring-rose-100 animate-pulse' : 'bg-white border-slate-200/40 hover:bg-emerald-50/50'"
                    class="flex flex-col items-center justify-center py-2.5 rounded-xl shadow-xs border group transition cursor-pointer"
                  >
                    <span class="text-lg group-hover:scale-110 transition duration-200" :class="{'text-rose-600': isListening}">🎤</span>
                    <span 
                      class="text-[10px] font-bold mt-1 uppercase tracking-wider"
                ...
                <div v-if="voiceTranscript" class="transition animate-fade-in">
                  <p class="text-[10px] text-indigo-600 font-semibold italic bg-indigo-50/60 p-2.5 rounded-xl border border-indigo-100/50 w-full text-center shadow-3xs">
                    "{{ voiceTranscript }}"
                  </p>
                </div>
                
                <hr class="border-t border-dashed border-slate-200/60 my-2" />
                ...
                <button type="submit" :disabled="formLoading" class="w-full py-2.5 px-4 rounded-xl text-xs font-black text-white bg-indigo-600 hover:bg-indigo-700 transition shadow-sm disabled:opacity-50 active:scale-98 cursor-pointer">
                  {{ formLoading ? 'Syncing...' : 'Save Manual Record Asset' }}
                </button>
              </form>
            </div>
